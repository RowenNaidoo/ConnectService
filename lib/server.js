...
router.post('/addNominee', function (request, response) {
    if ((0, _service.canAddNominee)(request.body)) {
...
router.get('/related-accounts', function (request, response) {
    var result = request.query.relationType ? (0, _service.getNominees)(parseInt(request.query.qffMemberId), request.query.relationType) : (0, _service.getNomineesByQffNo)(parseInt(request.query.qffMemberId));
    response.send({ "related-accounts": result });
});

router.delete('/connections/:connectionId', function (request, response) {
    (0, _service.disconnectNominee)(request.params.connectionId);
    response.send(200);
});
